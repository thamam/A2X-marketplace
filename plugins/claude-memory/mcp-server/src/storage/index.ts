...
export class StorageManager {
  ...
  async saveConversation(
    ...
    // Save to filesystem
    this.fs.saveConversation(conversation);
  ...
  private async enforceStorageLimits(): Promise<void> {
    ...
    if (stats.dbSizeMB > maxSizeMB) {
      console.warn(`Memory storage exceeds limit (${stats.dbSizeMB.toFixed(2)}MB / ${maxSizeMB}MB)`);
      console.warn('Consider archiving old memories or increasing the limit');
    }
  }
